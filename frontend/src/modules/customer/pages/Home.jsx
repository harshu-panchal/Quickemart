...
const Home = () => {
  ...
  const fetchData = async ({ forceRefresh = false } = {}) => {
    ...
    try {
      ...
      if (prodRes.data.success) {
        ...
        nextHomeData.products = dbProds.map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.salePrice || p.price, originalPrice: p.price, weight: p.weight || "1 unit", deliveryTime: "10-30 mins" }));
      }
  ...
  const hydrateSelectedSectionProducts = async (sections = []) => {
    ...
    try {
      ...
      const fetchedMissing = missingResults.filter((r) => r.status === "fulfilled").flatMap((r) => { const p = r.value?.data?.result || r.value?.data?.results; return Array.isArray(p) ? p : (p ? [p] : []); }).map((p) => ({ ...p, id: p._id, image: p.mainImage || p.image || "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=400&h=400", price: p.salePrice || p.price, originalPrice: p.price, weight: p.weight || "1 unit", deliveryTime: "10-30 mins" }));
  ...
  return (
    <div className={`min-h-screen pt-0 ${products.length === 0 && !isLoading ? "bg-white" : "bg-[#F5F7F8]"}`}>
